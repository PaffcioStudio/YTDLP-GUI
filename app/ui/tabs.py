# -*- coding: utf-8 -*-
"""
Inicjalizacja zakładek interfejsu użytkownika:
  init_video_tab, init_audio_tab, init_playlist_tab,
  init_queue_tab, init_advanced_tab, init_settings_tab
"""
...
def init_video_tab(win, tab_widget: QWidget):
    layout = QVBoxLayout(tab_widget)
    scroll = QScrollArea(); scroll.setWidgetResizable(True)
    content = QWidget(); scroll.setWidget(content)
    cl = QVBoxLayout(content)

    fmt_group = QGroupBox("Format wideo")
    fmt_lay = QFormLayout()

    win.video_format = QComboBox()
    win.video_format.addItems(["mp4", "webm", "mkv", "flv", "avi"])
    fmt_lay.addRow("Format kontenera:", win.video_format)
    ...
    fmt_lay.addRow("Jakość:", win.video_quality)

    win.video_codec = QComboBox()
    win.video_codec.addItems(["auto", "h264", "h265", "vp9", "av1"])
    fmt_lay.addRow("Kodek wideo:", win.video_codec)

    win.embed_thumbnails = QCheckBox("Osadź miniaturkę")
    win.embed_thumbnails.setChecked(True)
    fmt_lay.addRow(win.embed_thumbnails)

    win.embed_subs = QCheckBox("Osadź napisy")
    fmt_lay.addRow(win.embed_subs)

    fmt_group.setLayout(fmt_lay)
    cl.addWidget(fmt_group)

    opt_group = QGroupBox("Opcje pobierania")
    opt_lay = QFormLayout()

    win.output_template = QLineEdit("%(title)s.%(ext)s")
    opt_lay.addRow("Szablon nazwy pliku:", win.output_template)
    ...
    win.extract_audio   = QCheckBox("Wyodrębnij tylko audio")
    win.keep_video      = QCheckBox("Zachowaj plik wideo przy wyodrębnianiu audio")
    win.write_subs      = QCheckBox("Zapisz napisy do pliku")
    win.write_auto_subs = QCheckBox("Zapisz automatyczne napisy")
    win.write_info_json = QCheckBox("Zapisz metadane do pliku .info.json")
    for w in (win.extract_audio, win.keep_video, win.write_subs,
              win.write_auto_subs, win.write_info_json):
        extra_lay.addWidget(w)
    extra_group.setLayout(extra_lay)
    cl.addWidget(extra_group)
...
def init_audio_tab(win, tab_widget: QWidget):
    layout = QVBoxLayout(tab_widget)
    scroll = QScrollArea(); scroll.setWidgetResizable(True)
    content = QWidget(); scroll.setWidget(content)
    cl = QVBoxLayout(content)

    fmt_group = QGroupBox("Format audio")
    fmt_lay = QFormLayout()

    win.audio_format = QComboBox()
    win.audio_format.addItems(["najlepszy", "mp3", "aac", "flac", "m4a", "opus", "vorbis", "wav"])
    fmt_lay.addRow("Format:", win.audio_format)
    ...
    fmt_lay.addRow("Jakość (VBR):", win.audio_quality)

    win.add_metadata = QCheckBox("Dodaj metadane (artysta, tytuł itp.)")
    win.add_metadata.setChecked(True)
    fmt_lay.addRow(win.add_metadata)

    win.embed_thumbnail = QCheckBox("Osadź miniaturkę w pliku audio")
    win.embed_thumbnail.setChecked(True)
    fmt_lay.addRow(win.embed_thumbnail)

    fmt_group.setLayout(fmt_lay)
    cl.addWidget(fmt_group)

    opt_group = QGroupBox("Opcje pobierania audio")
    opt_lay = QFormLayout()

    win.audio_output_template = QLineEdit("%(title)s.%(ext)s")
    opt_lay.addRow("Szablon nazwy pliku:", win.audio_output_template)

    win.audio_output_path = QLineEdit()
    browse_aout = QPushButton(" Przeglądaj...")
    browse_aout.setIcon(win.style().standardIcon(QStyle.StandardPixmap.SP_DirOpenIcon))
    browse_aout.clicked.connect(win.browse_audio_output_path)
    aout_row = QHBoxLayout()
    aout_row.addWidget(win.audio_output_path); aout_row.addWidget(browse_aout)
    opt_lay.addRow("Katalog wyjściowy:", aout_row)

    opt_group.setLayout(opt_lay)
    cl.addWidget(opt_group)

    extract_group = QGroupBox("Opcje ekstrakcji audio")
    extract_lay = QVBoxLayout()
    win.prefer_ffmpeg = QCheckBox("Preferuj ffmpeg do scalania/konwersji")
    extract_lay.addWidget(win.prefer_ffmpeg)
    extract_group.setLayout(extract_lay)
    cl.addWidget(extract_group)
...
def init_playlist_tab(win, tab_widget: QWidget):
    layout = QVBoxLayout(tab_widget)
    scroll = QScrollArea(); scroll.setWidgetResizable(True)
    content = QWidget(); scroll.setWidget(content)
    cl = QVBoxLayout(content)

    pl_group = QGroupBox("Opcje playlisty")
    pl_lay = QFormLayout()

    win.playlist_start = QSpinBox(); win.playlist_start.setRange(1, 9999)
    pl_lay.addRow("Rozpocznij od filmu:", win.playlist_start)

    win.playlist_end = QSpinBox(); win.playlist_end.setRange(0, 9999)
    win.playlist_end.setSpecialValueText("Koniec")
    pl_lay.addRow("Zakończ na filmie:", win.playlist_end)

    win.playlist_items = QLineEdit()
    win.playlist_items.setPlaceholderText("np. 1,3,5-8 lub 1-3,7")
    pl_lay.addRow("Konkretne pozycje:", win.playlist_items)

    win.playlist_reverse = QCheckBox("Pobierz w odwrotnej kolejności")
    pl_lay.addRow(win.playlist_reverse)
    win.playlist_random = QCheckBox("Pobierz w losowej kolejności")
    pl_lay.addRow(win.playlist_random)

    pl_group.setLayout(pl_lay)
    cl.addWidget(pl_group)

    arch_group = QGroupBox("Opcje archiwum")
    arch_lay = QVBoxLayout()

    win.use_archive = QCheckBox("Użyj pliku archiwum (pomiń pobrane wcześniej)")
...
def init_queue_tab(win, tab_widget: QWidget):
    layout = QVBoxLayout(tab_widget)
    q_group = QGroupBox("Kolejka pobierania")
    q_lay = QVBoxLayout()

    win.queue_list = QueueListWidget(win)
    win.queue_list.setToolTip("Lista adresów URL w kolejce. Obsługuje drag&drop i menu kontekstowe.")
    ...
    clear_q_btn = QPushButton(" Wyczyść")
...
def init_advanced_tab(win, tab_widget: QWidget):
    layout = QVBoxLayout(tab_widget)
    scroll = QScrollArea(); scroll.setWidgetResizable(True)
    content = QWidget(); scroll.setWidget(content)
    ...
    win.extract_audio_adv = QCheckBox("Wyodrębnij audio (zaawansowane)")
    ...
    post_lay.addRow("Format audio:", win.audio_format_adv)
    win.audio_quality_adv = QComboBox()
    win.audio_quality_adv.addItems([str(i) for i in range(11)])
    post_lay.addRow("Jakość audio (VBR):", win.audio_quality_adv)
    win.keep_video_adv = QCheckBox("Zachowaj plik wideo po wyodrębnieniu (zaawansowane)")
    ...
    post_lay.addRow("Argumenty postprocesora (FFmpeg):", win.postprocessor_args)
    ...
    win.skip_download = QCheckBox("Pomiń pobieranie (tylko informacje)"); other_lay.addRow(win.skip_download)
    ...
    # --- Własne argumenty yt-dlp ---
    custom_group = QGroupBox("Własne argumenty yt-dlp")
    custom_lay = QVBoxLayout()
    custom_info = QLabel(
        "Dodatkowe argumenty przekazywane bezpośrednio do yt-dlp (dopisywane na końcu komendy, przed URL-em).\n"
        "Przykłady:  --cookies-from-browser chrome    --extractor-args \"generic:impersonate=chrome\"\n"
        "            --sub-lang pl  --convert-subs srt    --no-playlist"
    )
...
def init_settings_tab(win, tab_widget: QWidget):
    layout = QVBoxLayout(tab_widget)
    scroll = QScrollArea(); scroll.setWidgetResizable(True)
    content = QWidget(); scroll.setWidget(content)
    ...
    win.ytdlp_path_input.setPlaceholderText(
        f"Domyślnie: {YTDLP_PATH_WINDOWS} (Windows) lub 'yt-dlp' (PATH)"
    )
    ...
    win.ffmpeg_path.setPlaceholderText(
        f"Domyślnie: {FFMPEG_PATH_WINDOWS} (Windows) lub 'ffmpeg' (PATH)"
    )
    ...
    sl.addWidget(cda_group)

    # --- Domyślne ---
    def_group = QGroupBox("Domyślne ustawienia ogólne")
    def_lay = QFormLayout()
...
def init_about_tab(win, tab_widget: QWidget):
    layout = QVBoxLayout(tab_widget)
    ...
    links_lay = QFormLayout()
    links_lay.setHorizontalSpacing(16)
    ...
    links_lay.addRow(
        "yt-dlp:",
        _make_link_label("github.com/yt-dlp/yt-dlp",
                         "https://github.com/yt-dlp/yt-dlp"),
    )
    links_lay.addRow(
        "FFmpeg:",
        _make_link_label("github.com/FFmpeg/FFmpeg",
