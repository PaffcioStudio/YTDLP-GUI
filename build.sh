...
build_deb() {
    ...
    mkdir -p deb_build/${DEB_NAME}/DEBIAN
    mkdir -p deb_build/${DEB_NAME}/usr/bin
    mkdir -p deb_build/${DEB_NAME}/usr/share/applications
    mkdir -p deb_build/${DEB_NAME}/usr/share/pixmaps
    mkdir -p deb_build/${DEB_NAME}/usr/share/icons/hicolor/256x256/apps
    ...
    convert icon.ico deb_build/${DEB_NAME}/usr/share/pixmaps/ytdlp-gui.png
    convert icon.ico -resize 256x256 deb_build/${DEB_NAME}/usr/share/icons/hicolor/256x256/apps/ytdlp-gui.png
    
    # Tworzenie pliku control
    cat > deb_build/${DEB_NAME}/DEBIAN/control << EOF
Package: ytdlp-gui
Version: ${APP_VERSION}
Section: multimedia
Priority: optional
...
EOF
    ...
    cat > deb_build/${DEB_NAME}/DEBIAN/postinst << EOF
#!/bin/bash
...
EOF
    chmod +x deb_build/${DEB_NAME}/DEBIAN/postinst

    # Tworzenie skryptu postrm do czyszczenia po odinstalowaniu
    cat > deb_build/${DEB_NAME}/DEBIAN/postrm << EOF
#!/bin/bash
...
EOF
    chmod +x deb_build/${DEB_NAME}/DEBIAN/postrm
    
    # Tworzenie pliku .desktop
    cat > deb_build/${DEB_NAME}/usr/share/applications/ytdlp-gui.desktop << EOF
[Desktop Entry]
Type=Application
Name=YTDLP-GUI
...
EOF
    ...
    mkdir -p dist
    mv deb_build/${DEB_NAME}.deb dist/
    
    # Usuwanie zbędnej binarki PyInstaller z dist
    rm -f dist/YTDLP-GUI
    
    deactivate
    echo -e "${GREEN}DEB zbudowany: dist/${DEB_NAME}.deb${NC}"
}
...
EOF
    ...
    if command -v convert &> /dev/null; then
        ...
        echo -e "${YELLOW}ImageMagick nie znaleziony, używam ICO${NC}"
...
EOF
    ...
    deactivate
    echo -e "${GREEN}AppImage zbudowany: dist/YTDLP-GUI-${APP_VERSION}-x86_64.AppImage${NC}"
}
...
build_windows_on_linux() {
    ...
    echo -e "${RED}UWAGA: To jest funkcja eksperymentalna!${NC}"
...
build_with_wine() {
    ...
    if ! command -v wine &> /dev/null; then
        ...
        echo -e "${YELLOW}Wine jest wymagany do budowania Windows EXE na Linux${NC}"
        ...
        if [[ $wine_install == "t" || $wine_install == "T" ]]; then
            ...
            echo -e "${YELLOW} Anulowano budowanie Windows EXE${NC}"
            return 1
        fi
    fi
    ...
    if ! wine python --version &> /dev/null; then
        echo -e "${YELLOW}Python nie jest zainstalowany w Wine${NC}"
        echo -e "${BLUE} Automatyczna instalacja Python 3.12...${NC}"
        
        # Pobieranie Python
        if [ ! -f "python-installer.exe" ]; then
            echo -e "${BLUE} Pobieranie instalatora Python...${NC}"
        ...
        echo -e "${BLUE}Instalowanie Python w Wine...${NC}"
    ...
    # Instalacja pakietów (bez upgrade pip - Wine/Python 3.12 sypie się na CopyFile2)
    echo -e "${BLUE}Instalowanie pakietów w Wine...${NC}"
    wine venv_wine_build/Scripts/python.exe -m pip install PyQt6 requests pyinstaller
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -f dist/YTDLP-GUI.exe ]; then
            mv dist/YTDLP-GUI.exe dist/YTDLP-GUI-${APP_VERSION}-windows.exe 2>/dev/null
            echo -e "${GREEN}Windows EXE zbudowany: dist/YTDLP-GUI-${APP_VERSION}-windows.exe${NC}"
            
            # Wyświetlenie informacji o pliku
            EXE_SIZE=$(du -h dist/YTDLP-GUI-${APP_VERSION}-windows.exe | cut -f1)
    ...
    if [[ $cleanup_choice == "t" || $cleanup_choice == "T" || $cleanup_choice == "" ]]; then
        echo -e "${BLUE}Usuwanie plików tymczasowych...${NC}"
...
EOF
    ...
    if [ $? -eq 0 ]; then
        ...
        [ -f dist/YTDLP-GUI.exe ] && mv dist/YTDLP-GUI.exe dist/YTDLP-GUI-${APP_VERSION}-windows.exe
        [ -d dist/YTDLP-GUI-Portable ] && mv dist/YTDLP-GUI-Portable dist/YTDLP-GUI-${APP_VERSION}-Portable
    else
...
case $choice in
    ...
    3)
        ...
        rm -f dist/YTDLP-GUI
        deactivate
        echo -e "${GREEN}Linux executable: dist/YTDLP-GUI-${APP_VERSION}-linux${NC}"
        cleanup_build_dirs
        ;;
    4)
